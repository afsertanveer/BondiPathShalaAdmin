...
const StudentHistory = () => {
  ...
  useEffect(() => {
    setIsLoading(true);
    axios
      .get("/api/student/historydataadmin?studentId=" + studentId)
      .then(({ data }) => {
                    ...
                    >
                      ...
                      <td>
                        <div className="flex px-2 justify-evenly">
                          <Link
                            to={`/dashboard/exams/${studentId}/${data.examId}/solution`}
                            className="tooltip bg-color-two rounded-full text-center h-[38px] w-[38px]"
                            data-tip="Get Solution"
                          >
                            <img
                              className="inline-flex img p-2"
                              src={v2}
                              alt="view-solution"
                            />
                          </Link>
