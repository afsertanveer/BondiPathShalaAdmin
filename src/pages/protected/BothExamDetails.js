...
const BothExamDetails = () => {
  ...
  const handlePageClick = (event) => {
    ...
    if (event.selected > 0) {
      axios
        .get(`/api/student/bothgethistory?examId=${selectedExam}&page=${clickedPage}`)
        .then(({ data }) => {
          console.log(data);
      ...
      axios
      .get(`/api/student/bothgethistory?examId=${selectedExam}&page=${1}`)
      .then(({ data }) => {
        console.log(data);
        setDetailedExam(data?.data);
        setExamInfo(data.examInfo)
...
export default BothExamDetails;