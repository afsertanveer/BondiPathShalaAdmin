...
const ShowFreeExam = () => {
  ...
  const recalculate = (id) => {
    axios
      .post(`/api/exam/calculatemartks?type=100`, {
        examId: singleExamId,
      })
      .then((data) => {
        axios
  ...
  const generator = (id) => {
    axios
      .put(`/api/freestudent/updatestudentexaminfofree?examId=${id}`)
      .then(({ data }) => {
        if (data.length === 0) {
          ...
          axios
            .post(`/api/freestudent/updaterankfree?examId=${id}`)
            .then((rankData) => {
  ...
  const addPublish = (examId) => {
    axios
      .post(`/api/freestudent/addpublishfree`, { examId })
      .then(({ data }) => {
  ...
  const changeStatus = (examId, status) => {
    axios
      .post(`/api/freestudent/statusupdatepublishfree`, {
        examId,
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-center">
                      <div className="flex flex-col lg:flex-row justify-center">
                        ...
                        <label
                          onClick={() =>
                            assignIdstatus(exam._id, !exam.publishStatus)
                          }
