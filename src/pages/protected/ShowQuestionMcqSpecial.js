...
const ShowQuestionMcqSpecial = () => {
  ...
  const handleChangeSecondCourse = (e) => {
    ...
    axios
      .get(`/api/mcqspecialexam/showmcqspecialexambycourse?courseId=${e.target.value}`)
      .then(({ data }) => {
        // console.log(data);
  ...
  const handleChangeSet = (setName) => {
    ...
    if (parseInt(setName) !== -1) {
      axios
        .get(
          `/api/mcqspecialexam/questionByExamIdSubjectAndSet?examId=${selectedExam}&subjectId=${selectedSubject}&setName=${parseInt(
            setName
          )}`
        )
        .then(({ data }) => {
  ...
  return (
    <div className=" px-0 lg:px-4">
      <div className="flex w-full justify-center items-center py-5 px-2 my-5  ">
        <div className="bg-white px-1 lg:px-2   py-2 grid grid-cols-1 lg:grid-cols-4 gap-3">
...
export default ShowQuestionMcqSpecial
