...
const SearchStudent = () => {
  ...
  const handleSearch = (e) => {
    ...
    if (searched.length > 2) {
      ...
      if (selectedOption === '1') {
        axios
          .get(`/api/student/getstudentregsearch?regNo=${searched}`)
          .then(({ data }) => {
            setIsLoading(false);
            setStudents(data);
            flag = 1;
      ...
      if (selectedOption === '2') {
        axios
          .get(`/api/student/getstudentnamesearch?name=${searched}`)
          .then(({ data }) => {
            setIsLoading(false);
            setStudents(data);
            flag = 1;
    ...
    if (searched.length > 4) {
      if (selectedOption === '3') {
        axios
          .get(`/api/student/getstudentmobilesearch?mobileNo=${searched}`)
          .then(({ data }) => {
            setIsLoading(false);
            setStudents(data);
          })
              ...
              {students.length > 0 &&
                students.map((student) => (
                  ...
                  >
                    ...
                    <td className='px-6 py-4 text-center'>
                      {student.hscRoll === null ? 'N/A' : student.hscRoll}
                    </td>
                    <td className='px-6 py-4 text-center'>
                      {student.sscRoll === null ? 'N/A' : student.sscRoll}
                    </td>
                    <td className='px-6 py-4 text-center'>
                      <button
                        className='btn'
                        onClick={() => setData(student._id, student.regNo)}
                      >
                        Reset Password
                      </button>
                    </td>
