    ...
    <div className="mb-8 pb-2 rounded-lg shadow-[0px_0px_6px_2px_rgba(0,0,0,0.5)] relative">
      ...
      <div className="mb-1">
        {question.type == false ? (
          ...
          <div className="rounded-lg text-lg font-bold p-4">{question.question}</div>
          )}
      </div>
      ...
      <ul className="my-3 flex space-x-4 justify-center">
        {
          options.map((opt, idx) => {
            ...
            return (<li className={`mb-0 px-4 py-2 border-4 bg-[#262626] rounded-lg ${defaultTextColor}`} key={`soln.${idx}`}>
